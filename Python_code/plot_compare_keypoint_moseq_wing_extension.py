# -*- coding: utf-8 -*-
"""
GT / YORU / KM を統合して、120–240 s の 0/1 エソグラムを描画。
上から GT → YORU → KM (F1上位TOP_Nシラブル or 先行CSVの順)。
- 全ライン黒、枠なし、x軸非表示
- 背景ハイライト（GT=1 区間）は設定でオン/オフ
- YORU: class_name に 'wing_extension' があれば 1（重複frameはOR集約）
- 追加: 先に作成した syllable ランキング CSV（rank, syllable, f1）を読み込み、
        その順番で KM の行を並べる。見つからなければ従来のF1計算にフォールバック。
"""
...
# ===== 設定 =====
GT_CSV   = "./data/ground_truth/cs-h_movie_0015 23-10-27 15-40-40_cropped.csv"
KM_CSV   = "./data/keypoint_moseq/cs-h_movie_0015 23-10-27 15-40-40.csv"
YORU_CSV = "./data/yoru/cs-h_movie_0015 23-10-27 15-40-40.csv"
...
OUT_IMG = "./outputs/fly_ethogram_top10_120-240s_ranked.pdf"
SCALE_BAR_SEC = 10.0

# 先に作成したランキング CSV。空文字("")なら OUT_IMG と同じフォルダから *_syllables_by_f1.csv を自動探索し最も新しいものを使用
SYLLABLE_RANKING_CSV = "./outputs/plots_dataset_bars_pdf_color/fly_wingextension_syllables_by_f1.csv"  # 例: "./outputs/cs-h_movie_0015_syllables_by_f1.csv"

# 背景ハイライト（GT=1 の時間帯）のオン/オフと見た目
HIGHLIGHT_GT    = False          # ← False でハイライトなし
...
def load_gt(gt_path):
    ...
    if beh_col is None:
        for c in df.columns:
            if str(c).strip().lower() in ["wing_extension", "behavior", "behaviour", "state", "label"]:
...
def parse_class_name_to01(x):
    ...
    if (s_low.startswith("[") and s_low.endswith("]")) or (s_low.startswith("(") and s_low.endswith(")")):
        try:
            ...
            return 1 if any("wing_extension" in t for t in tokens) else 0
        except Exception:
            pass
    for sep in [",", ";", "|", "/"]:
        if sep in s:
    ...
    if " " in s.strip():
        ...
        return 1 if any("wing_extension" in t for t in tokens) else 0
    return 1 if "wing_extension" in s_low else 0

def load_yoru(yoru_path, fps):
    """class_name に 'wing_extension' があれば 1。frame が重複する場合はフレーム単位で OR 集約。"""
...
if order_from_csv is not None:
    ...
    # フォールバック：F1（GT基準）で上位 TOP_N を選定（従来ロジック）
...
y_labels.append(f"Manual Annotation")
y -= row_gap
...
if order_from_csv is None:
    # フォールバック時は参考として上位リストを出力
    print("\nTop-by-F1 (120–240 s):")
