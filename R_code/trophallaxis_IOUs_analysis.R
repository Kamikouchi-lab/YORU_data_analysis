  ...
  #read to CSV
  data_path = "../datas/iou_ant_tro_data.csv"
  ...
  # Only trophallaxis
  df_tro_data = filter(df_data, class == "0")
  
  gp_tro = ggplot(data = df_tro_data, aes(x = train_no, y = value, fill = models)) + 
    # geom_boxplot() +
    ...
    labs(title="ant - trophallaxis", x = "Training images", y = "IOU")+
  ...
  gp_v5s = ggplot(data = df_data_filtered, aes(x = train_no, y = value)) + 
    ...
    labs(title="Ant tro model", x = "Training images", y = "Value")+
    scale_color_discrete(name = element_blank(), labels = text_label_legend)+
    Setting
  gp_v5s
  ...
  ggsave("../graph/model_evaluation_iou_tro.png", gp_tro, width = 12, height = 3, dpi = 500)
  ggsave("../graph/model_evaluation_iou_tro.svg", gp_tro, width = 12, height = 3, dpi = 500)