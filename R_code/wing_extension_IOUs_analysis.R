...
gp_all = ggplot(data = df_data, aes(x = train_no, y = value, fill = models)) + 
  # geom_boxplot() +
  ...
  # facet_wrap(? train_no, labeller = labeller(class = class_labs), scales = "free_x") +
  labs(title="fly - wing extension", x = "Training images", y = "IOU")+
...
gp_v5s = ggplot(data = df_data_filtered, aes(x = train_no, y = value)) + 
  ...
  geom_violin() +
  # geom_line()+
  facet_grid(. ? class, labeller = labeller(class = class_labs)) +
  labs(title="fly - wing extension", x = "Training images", y = "Value")+
  scale_color_discrete(name = element_blank(), labels = text_label_legend)+
  Setting
gp_v5s


ggsave("../graph/model_evaluation_iou_wing_extesnsion.png", gp_all, width = 12, height = 4, dpi = 500)
ggsave("../graph/model_evaluation_iou_wing_extesnsion.svg", gp_all, width = 12, height = 4, dpi = 500)
