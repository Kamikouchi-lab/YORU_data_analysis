...
data <- readr::read_csv(data_csv_path) %>%
  dplyr::filter(condition== "ATR") %>%
  dplyr::filter(group != "w1118_ATR_pattern")
data <- transform(data, group = factor(group, levels = c("jo15_ATR_spe", "jo15_ATR_pattern", "w1118_ATR_spe")))
...
###Analyze RMTL###
data$surv_obj <- with(data, Surv(latency, event))
tau <- 1800 # analysis limit time (sec)
fit <- survfit(surv_obj ? group, data = data)
# print(rmst_res)
# rmst_res <- rmst2(surv_obj1group, data = data, tau = tau)
# rmst_res <- rmst2(time = data$latency, status = data$event, arm = data$group)
...
# creating graph
gp <- ggplot(data, aes(x=latency, colour=group)) +
...
time_limit = 1800
data$limited_latency = pmin(data$latency, time_limit)
data$status_limited = as.integer(data$latency <= time_limit & data$event == 1)
